...
const DistrictAdminDetails = () => {
    ...
    // Directory States (For Dropdowns)
    const [locationData, setLocationData] = useState([]);
    const [banks, setBanks] = useState([]);
    const [districts, setDistricts] = useState({ p: [], t: [], o: [], jur: [] });
        ...
        try {
            ...
            if (detailsRes.data.success) {
                ...
                setDistricts({
                    ...
                    jur: getDists(master.assignedState)
                });
                setBlocks({
            ...
            toast.error("District Node Handshake Failed.");
        } finally {
    ...
    const handleUpdate = async () => {
        try {
            ...
            if (res.data.success) {
                toast.success("District Records Synchronized.");
                setIsEditing(false);
    ...
    const handlePasswordReset = async () => {
        const newPass = window.prompt("🔐 SECURITY: Enter New Master Password for District Admin:");
        if (!newPass) return;
        try {
    ...
    const handlePurge = async () => {
        if (window.confirm("🚨 CRITICAL: Decommission node permanently?") && window.prompt("Type 'DELETE':") === 'DELETE') {
            try {
                ...
                toast.error("District Node Purged.");
                navigate('/admin/district-mgmt');
    ...
    const handleDeviceAction = async (deviceId, currentAuth) => {
        try {
            ...
            toast.success("Device Authorization Updated.");
            fetchEverything(true);
    ...
    const handleLocChange = (section, level, value) => {
        const pfx = section === 'jur' ? 'assigned' : section;
        if (level === 'state') {
            ...
            if(section !== 'jur') setBlocks(prev => ({ ...prev, [section]: blks }));
    ...
    if (loading && !formData) return <div style={loaderS}><div className="rkd-main-spinner"></div><p style={{marginTop:'15px', fontWeight:'900'}}>📡 SYNCING DISTRICT NODE...</p></div>;

    const themeColor = settings?.themeColor || '#0f172a';
    const permIcons = { localDashboard: '📊', shopAudit: '🏪', inventoryCheck: '📦', deliveryLogistics: '🚚', districtSupport: '🎧', payoutVerification: '💳' };
    ...
    return (
        <div style={containerS}>
            {/* --- HEADER --- */}
            <div style={headerS}>
                <button onClick={() => navigate('/admin/district-mgmt')} style={backBtnS}>← REGISTRY INDEX</button>
                <div style={{flex: 1}}>
                    <h2 style={{margin:0, color: themeColor}}>📍 DISTRICT ADMIN: {formData?.generatedId}</h2>
                    <div style={statusRowS}>
                        <span style={dotS(formData?.isActive)}></span>
                        <span style={roleTagS}>{formData?.role} • {formData?.assignedDistrict}, {formData?.assignedState}</span>
                    </div>
                </div>
                <div style={actionGroupS}>
                    ...
                    {!isEditing ? (
                        <button onClick={() => setIsEditing(true)} style={primaryBtnS(themeColor)}>📝 EDIT RECORDS</button>
                    ) : (
            ...
            <div style={metricsBarS}>
                <MetricCard label="Approved Shops" val={stats.approved} icon="🏪" color="#10b981" />
                ...
                <MetricCard label="Node Health" val={stats.uptime} icon="⚡" color="#f59e0b" />
            </div>

            <div style={mainGridS}>
                {/* --- LEFT COL --- */}
                <div style={colS}>
                    <Section title="📍 LOCAL JURISDICTION" color="#e67e22">
                        <div style={grid2}>
                            <Input label="Assigned State" name="assignedState" val={formData?.assignedState} edit={isEditing} type="select" options={locationData.map(l=>l.state)} onChange={(e)=>handleLocChange('jur','state',e.target.value)} />
                            <Input label="Assigned District" name="assignedDistrict" val={formData?.assignedDistrict} edit={isEditing} type="select" options={districts.jur.map(d=>d.name)} onChange={(e)=>handleLocChange('jur','district',e.target.value)} />
                        </div>
                    </Section>
                    ...
                    <Section title="📑 STATUTORY & KYC" color="#e74c3c">
                        <div style={grid2}>
                            ...
                            <Input label="Experience (Yrs)" name="experienceYears" val={formData?.experienceYears} edit={isEditing} onChange={handleFieldChange} />
                        </div>
                    </Section>

                    <Section title="💳 BANKING Hub" color="#27ae60">
                        <div style={grid2}>
                ...
                {/* --- RIGHT COL --- */}
                <div style={colS}>
                    <Section title="🏠 ADDRESS HUB (3-LAYER)" color="#3498db">
                        ...
                        <AddressBox label="DISTRICT HQ OFFICE" prefix="office" data={formData} edit={isEditing} locationData={locationData} districts={districts.o} blocks={blocks.o} onLocChange={handleLocChange} onFieldChange={handleFieldChange} />
                    </Section>
                            ...
                            {formData?.permissions && Object.keys(formData.permissions).map(p => (
                                <div key={p} onClick={() => handleInstantPermissionToggle(p)} style={permNodeS(formData.permissions[p])}>
                                    ...
                                    <div style={{flex:1}}>
                                        ...
                                        <div style={{fontSize:'7px', color: formData.permissions[p] ? '#10b981' : '#f43f5e', fontWeight:'800'}}>{formData.permissions[p] ? 'ACTIVE' : 'LOCKED'}</div>
                                    </div>
                    ...
                    <Section title="📱 AUTHORIZED LOGIN DEVICES" color="#64748b">
                        ...
                        )) : <div style={emptyS}>No hardware nodes bound.</div>}
                    </Section>

                    <Section title="📝 PRIVATE SYSTEM NOTES" color="#94a3b8">
                        <textarea style={notesInS} placeholder="Internal audit notes..." value={internalNotes} onChange={(e)=>setInternalNotes(e.target.value)} />
                    </Section>
...
// --- Helper Components & Style Objects ---
...
const AddressBox = ({ label, prefix, data, edit, locationData, districts, blocks, onLocChange, onFieldChange }) => (<div style={addrCardS}><label style={labS}>{label} LOCATION</label>{edit ? <div style={{marginTop:'8px'}}><div style={grid2}><select style={inS} value={data[`${prefix}State`] || ''} onChange={(e)=>onLocChange(prefix, 'state', e.target.value)}><option value="">State</option>{locationData.map(l => <option key={l.state} value={l.state}>{l.state}</option>)}</select><select style={inS} value={data[`${prefix}District`] || ''} onChange={(e)=>onLocChange(prefix, 'district', e.target.value)}><option value="">Dist</option>{districts?.map(d => <option key={d.name} value={d.name}>{d.name}</option>)}</select></div><div style={{...grid2, marginTop:'8px'}}><select style={inS} value={data[`${prefix}Block`] || ''} name={`${prefix}Block`} onChange={onFieldChange}><option value="">Block</option>{blocks?.map(b => <option key={b} value={b}>{b}</option>)}</select><input name={`${prefix}Pin`} placeholder="Pin" style={inS} value={data[`${prefix}Pin`] || ''} onChange={onFieldChange} /></div><textarea name={`${prefix}FullAddress`} style={{...inS, marginTop:'8px', height:'40px'}} value={data[`${prefix}FullAddress`] || ''} onChange={onFieldChange} placeholder="Full Address"/></div> : <p style={valS}>{data[`${prefix}FullAddress`]}, {data[`${prefix}Block`]}, {data[`${prefix}District`]}, {data[`${prefix}State`]} - {data[`${prefix}Pin`]}</p>}</div>);
...
const permGridS = { display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(160px, 1fr))', gap:'10px' };
...
export default DistrictAdminDetails;