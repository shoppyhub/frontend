...
const StateAdminDetails = () => {
    ...
    const fetchEverything = useCallback(async (isSilent = false) => {
        ...
        try {
            ...
            toast.error("Handshake Failed: Node unreachable.");
        } finally {
    ...
    const handlePasswordReset = async () => {
        const newPass = window.prompt("🔐 SECURITY: Enter New Master Password:");
        if (!newPass) return;
        try {
    ...
    const handleToggleStatus = async () => {
        const action = formData.isActive ? 'SUSPEND ACCESS' : 'ACTIVATE NODE';
        if (!window.confirm(`⚠️ Confirm: ${action}?`)) return;
        try {
            await api.patch(`/admin/hierarchy/status/${id}`, { isActive: !formData.isActive });
            setFormData(p => ({ ...p, isActive: !p.isActive }));
            toast.success(`Node is now ${!formData.isActive ? 'ACTIVE' : 'INACTIVE'}.`);
        } catch (err) { toast.error("Status Update Failed."); }
    };

    const handlePurge = async () => {
        if (window.confirm("🚨 CRITICAL: Purge permanently?") && window.prompt("Type 'DELETE':") === 'DELETE') {
            try {
                await api.delete(`/admin/hierarchy/remove/${id}`);
                toast.error("Node Purged.");
                navigate('/admin/state-mgmt');
    ...
    // New: Master Permission Toggle (Grant/Revoke All)
    const handleToggleAllPermissions = async () => {
        ...
        const targetValue = !allActive; // अगर सब एक्टिव हैं तो सबको बंद करें, वरना सबको चालू करें
        ...
        try {
            await api.put(`/admin/hierarchy/update/${id}`, { permissions: updatedPermissions });
            toast.success(targetValue ? "ALL PRIVILEGES GRANTED" : "ALL PRIVILEGES REVOKED");
        } catch (err) {
    ...
    const handleFieldChange = (e) => {
        ...
        if (name === 'panNumber' || name === 'bankIfsc') finalValue = finalValue.toUpperCase();
    ...
    if (loading && !formData) return <div style={loaderS}><div className="rkd-main-spinner"></div><p style={{marginTop:'15px', fontWeight:'900'}}>📡 SYNCING COMPLETE STATE NODE...</p></div>;

    const themeColor = settings?.themeColor || '#0f172a';
    const permIcons = { regionalOps: '🏙️', districtControl: '📍', merchantOnboarding: '🏪', financialAudit: '💰', stateLogistics: '🚚', grievanceRedressal: '🎧' };

    // Check if all perms are active for the button label
    ...
    return (
        <div style={containerS}>
            {/* --- HEADER --- */}
            <div style={headerS}>
                <button onClick={() => navigate('/admin/state-mgmt')} style={backBtnS}>← REGISTRY INDEX</button>
                <div style={{flex: 1}}>
                    <h2 style={{margin:0, color: themeColor}}>🏛️ STATE ADMIN: {formData?.generatedId}</h2>
                    <div style={statusRowS}>
                        <span style={dotS(formData?.isActive)}></span>
                        <span style={roleTagS}>{formData?.role} UNIT • {formData?.assignedState}</span>
                    </div>
                </div>
                <div style={actionGroupS}>
                    ...
                    <button onClick={handleToggleStatus} style={secActionBtnS(formData?.isActive ? '#f59e0b' : '#10b981')}>
                        {formData?.isActive ? '🚫 SUSPEND ACCESS' : '✅ ACTIVATE NODE'}
                    </button>
                    <button onClick={handlePurge} style={secActionBtnS('#ef4444')}>🗑️ PERMANENT DELETE</button>
                    {!isEditing ? (
                        <button onClick={() => setIsEditing(true)} style={primaryBtnS(themeColor)}>📝 EDIT ALL RECORDS</button>
                    ) : (
            ...
            <div style={metricsBarS}>
                <MetricCard label="Approvals" val={stats.approved} icon="✅" color="#10b981" />
                <MetricCard label="Pending Tasks" val={tasks.length} icon="📋" color="#3b82f6" onClick={() => navigate('/admin/requests')} />
                <MetricCard label="Rejected" val={stats.rejected} icon="❌" color="#f43f5e" />
                <MetricCard label="Login Nodes" val={formData?.authorizedDevices?.length || 0} icon="📱" color="#8b5cf6" />
            </div>
            ...
            <div style={mainGridS}>
                ...
                <div style={colS}>
                    <Section title="👤 PERSONAL & IDENTITY" color={themeColor}>
                        <div style={profileHeadS}>
                            <div style={{position:'relative', cursor:'zoom-in'}} onClick={() => !isEditing && setViewImage(formData?.adminPhoto)}>
                                <img src={formData?.adminPhoto || 'https://via.placeholder.com/150'} style={avatarS} alt="P" />
                                {isEditing && <input type="file" onChange={(e)=>{
                                    const reader = new FileReader();
                                    reader.readAsDataURL(e.target.files[0]);
                                    reader.onloadend = () => setFormData(p=>({...p, adminPhoto: reader.result}));
                                }} style={fileOverlayS} />}
                            </div>
                            <div style={{flex:1}}>
                        ...
                        <div style={grid2}>
                            <Input label="Date of Birth" name="dob" val={isEditing ? formData?.dob?.split('T')[0] : formatDate(formData?.dob)} edit={isEditing} type={isEditing ? "date" : "text"} onChange={handleFieldChange} />
                            <Input label="Jurisdiction State" name="assignedState" val={formData?.assignedState} edit={isEditing} type="select" options={locationData.map(l=>l.state)} onChange={handleFieldChange} />
                        </div>
                    </Section>

                    <Section title="📱 COMMUNICATION CHANNELS" color="#3498db">
                        <div style={grid2}>
                            <Input label="Primary Mobile" val={formData?.mobile} />
                            <Input label="Official Email" val={formData?.primaryEmail} />
                            <Input label="WhatsApp ID" name="whatsapp" val={formData?.whatsapp} edit={isEditing} onChange={handleFieldChange} />
                            <Input label="Secondary Email" name="secondaryEmail" val={formData?.secondaryEmail} edit={isEditing} onChange={handleFieldChange} />
                        </div>
                        <div style={subBorderS}>
                            <label style={subLabS}>EMERGENCY PROTOCOL</label>
                            <div style={grid2}>
                                <Input label="Emergency Contact Name" name="emergencyContactName" val={formData?.emergencyContactName} edit={isEditing} onChange={handleFieldChange} />
                                <Input label="Emergency Contact Mobile" name="emergencyContactNumber" val={formData?.emergencyContactNumber} edit={isEditing} onChange={handleFieldChange} />
                            </div>
                        </div>
                    </Section>
                    ...
                    <Section title="🎓 PROFESSIONAL DATA" color="#f39c12">
                        <div style={grid2}>
                            <Input label="Qualification" name="qualification" val={formData?.qualification} edit={isEditing} type="select" options={['Graduate','Post-Graduate','Doctorate']} onChange={handleFieldChange} />
                            <Input label="Field of Study" name="fieldOfStudy" val={formData?.fieldOfStudy} edit={isEditing} onChange={handleFieldChange} />
                            <Input label="Exp (Years)" name="experienceYears" val={formData?.experienceYears} edit={isEditing} onChange={handleFieldChange} />
                            <Input label="Previous Org" name="previousOrg" val={formData?.previousOrg} edit={isEditing} onChange={handleFieldChange} />
                        </div>
                    </Section>

                    <Section title="💳 BANKING SETTLEMENT" color="#27ae60">
                        <div style={grid2}>
                            ...
                            <Input label="IFSC Protocol" name="bankIfsc" val={formData?.bankIfsc?.toUpperCase()} edit={isEditing} onChange={handleFieldChange} />
                ...
                <div style={colS}>
                    <Section title="🏠 ADDRESS HUB (3-LAYER)" color="#e67e22">
                        ...
                        <AddressBox label="OFFICIAL OFFICE HQ" prefix="office" data={formData} edit={isEditing} locationData={locationData} districts={districts.o} blocks={blocks.o} onLocChange={handleLocChange} onFieldChange={handleFieldChange} />
                    </Section>

                    {/* --- GOVERNANCE PRIVILEGES WITH MASTER TOGGLE --- */}
                    <Section 
                    ...
                    >
                        <div style={{display:'flex', justifyContent:'flex-end', marginBottom:'15px'}}>
                            <button 
                                ...
                                style={masterToggleBtnS(areAllPermsActive)}
                            >
                                {areAllPermsActive ? '⛔ REVOKE ALL ACCESS' : '🔓 GRANT ALL ACCESS'}
                            </button>
                        </div>
                        <div style={permGridS}>
                            {formData?.permissions && Object.keys(formData.permissions).map(p => (
                                <div key={p} onClick={() => handleInstantPermissionToggle(p)} style={permNodeS(formData.permissions[p])}>
                                    ...
                                    <div style={{flex:1}}>
                                        <div style={{fontSize:'10px', fontWeight:'900'}}>{permIcons[p] || '🛡️'} {p.replace(/([A-Z])/g, ' $1').toUpperCase()}</div>
                                        <div style={{fontSize:'8px', color: formData.permissions[p] ? '#10b981' : '#f43f5e', fontWeight:'700'}}>{formData.permissions[p] ? 'POWER ACTIVE' : 'POWER LOCKED'}</div>
                                    </div>
                    ...
                    <Section title="📱 DEVICE SECURITY" color="#6366f1">
                        {formData?.authorizedDevices?.length > 0 ? formData.authorizedDevices.map((d, i) => (
                            <div key={i} style={listItemS}>
                                <div style={{flex:1}}><div style={listTitleS}>{d.deviceName}</div><div style={listSubS}>{d.ip} • {formatDate(d.lastLogin)}</div></div>
                                <button onClick={()=>handleDeviceAction(d.deviceId, d.isAuthorized)} style={d.isAuthorized ? revokeBtnS : allowBtnS}>
                                    {d.isAuthorized ? 'REVOKE ACCESS' : 'AUTHORIZE'}
                                </button>
                            </div>
                        )) : <div style={emptyS}>No devices bound.</div>}
                    </Section>

                    <Section title="📝 PRIVATE ADMIN NOTES" color="#94a3b8">
                        <textarea style={notesInS} placeholder="Internal system notes..." value={internalNotes} onChange={(e)=>setInternalNotes(e.target.value)} />
                    </Section>

                    <Section title="📜 RECENT AUDIT LOGS" color="#000">
                        <div style={logBoxS}>
            ...
            <Section title="📂 SECURITY DOCUMENT VAULT" color="#000">
                <div style={docVaultGridS}>
                    <DocPreview label="Aadhaar ID" file={formData?.aadharFile} onUpdate={(e)=>setFormData({...formData, aadharFile: e})} edit={isEditing} onZoom={()=>setViewImage(formData?.aadharFile)} />
                    <DocPreview label="PAN ID" file={formData?.panFile} onUpdate={(e)=>setFormData({...formData, panFile: e})} edit={isEditing} onZoom={()=>setViewImage(formData?.panFile)} />
                    ...
                    <DocPreview label="Passbook/Cheque" file={formData?.bankFile} onUpdate={(e)=>setFormData({...formData, bankFile: e})} edit={isEditing} onZoom={()=>setViewImage(formData?.bankFile)} />
                </div>
            </Section>
                ...
                <div style={modalOverlayS} onClick={() => { setViewImage(null); setImgRotation(0); }}>
                    <div style={modalContentS} onClick={(e)=>e.stopPropagation()}>
                        <div style={modalToolbarS}>
                            <button onClick={rotateImg} style={rotateBtnS}>🔄 ROTATE DOCUMENT</button>
                            <button onClick={() => { setViewImage(null); setImgRotation(0); }} style={closeModalS}>CLOSE (X)</button>
                        </div>
                        <img src={viewImage} style={{...imgLargeS, transform:`rotate(${imgRotation}deg)`}} alt="z" />
                    </div>
                </div>
            )}
        </div>
...
const masterToggleBtnS = (active) => ({
    background: active ? '#fff1f2' : '#f0fdf4',
    color: active ? '#f43f5e' : '#16a34a',
    border: active ? '1.5px solid #fecaca' : '1.5px solid #bbf7d0',
    padding: '8px 15px',
    borderRadius: '10px',
    fontSize: '9px',
    fontWeight: '900',
    ...
    <div style={addrCardS}>
        ...
        {edit ? <div style={{marginTop:'8px'}}>
            ...
            <div style={{...grid2, marginTop:'8px'}}>
                <select style={inS} value={data[`${prefix}Block`] || ''} name={`${prefix}Block`} onChange={onFieldChange}>
                    <option value="">Block</option>{blocks?.map(b => <option key={b} value={b}>{b}</option>)}
                </select>
                <input name={`${prefix}Pin`} placeholder="Pin" style={inS} value={data[`${prefix}Pin`] || ''} onChange={onFieldChange} />
            </div>
            <textarea name={`${prefix}FullAddress`} style={{...inS, marginTop:'8px', height:'40px'}} value={data[`${prefix}FullAddress`] || ''} onChange={onFieldChange} placeholder="Full Address"/>
        </div> : <p style={valS}>{data[`${prefix}FullAddress`]}, {data[`${prefix}Block`]}, {data[`${prefix}District`]}, {data[`${prefix}State`]} - {data[`${prefix}Pin`]}</p>}
    </div>
);

const MetricCard = ({ label, val, icon, color, onClick }) => (
    <div onClick={onClick} style={{background:'#fff', padding:'20px', borderRadius:'15px', display:'flex', alignItems:'center', gap:'12px', border:'1px solid #f1f5f9', flex:1, cursor: onClick?'pointer':'default'}}>
        ...
        <div><div style={{fontSize:'14px', fontWeight:'900'}}>{val}</div><div style={{fontSize:'9px', color:'#94a3b8', textTransform:'uppercase'}}>{label}</div></div>
    </div>
);
...
const DocPreview = ({ label, file, edit, onUpdate, onZoom }) => (
    <div style={docCardS}>
        ...
        <div style={docFrameS} onClick={onZoom}>
            {file ? <img src={file} style={{width:'100%', height:'100%', objectFit:'cover'}} alt="doc" /> : "MISSING"}
        </div>
...
export default StateAdminDetails;